...
OUT_EMP =  SELECT EMPNO,
         ...
         case
                   ...
                   HIREDATE as InvDate,
                   CASE
                   WHEN weekday( HIREDATE ) = 0 THEN 'MONDAY'
                   WHEN weekday( HIREDATE ) = 1 THEN 'TUESDAY'
                   WHEN weekday( HIREDATE ) = 2 THEN 'WEDNESDAY'
                   WHEN weekday( HIREDATE ) = 3 THEN 'THURSDAY'
                   WHEN weekday( HIREDATE ) = 4 THEN 'FRIDAY'
                   WHEN weekday( HIREDATE ) = 5 THEN 'SATURDAY'
                   WHEN weekday( HIREDATE ) = 6 THEN 'SUNDAY'
                   ELSE 'NA'
                   END AS InvDAY
       From Z104329EMP ;
